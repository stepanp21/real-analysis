...
:::{.exercise #exr-graph-and-range}
For each sequence below, make a sketch of both its range on the number line, and its graph in $\R^2$. Which visualization do you find more helpful for understanding what's going on in each case?

1. The sequence $\left(\frac{(-1)^n}{n}\right)$.
2. The sequence $0,1,0,2,0,3,0,4,\ldots$
:::
...
4. **monotonic** if either it is increasing or it is decreasing, and **strictly monotonic** if it is strictly increasing or if it is strictly decreasing.
...
We will do a proof by induction or by using the Well-Ordering Principle (@ax-wop).

...
:::{.axiom #ax-ordered-field}
$\R$ is an ordered field.
:::

Again, the properties listed in @def-ordered-field are not surprising; we already knew they were true for $\R.$ But more surprising is the fact the everything else we know about how inequalities behave and interact with arithmetic can be prove from @ax-field and @ax-ordered-field.
...
You are probably familiar with the basics of how inequalities and arithmetic interact, and henceforth, we can use them at will without citation. In this class, being fluent with the rules for inequalities will be essential, though, so we collect a list of "inequality rules" (all of which could in principle be proven from @ax-field and @ax-ordered-field) in @sec-inequality-rules.

Worth reflecting on now is where we stand on our stated goals in creating the axioms. The @ax-ordered-field was successful in distinguishing $\R$ from $\mathbb{F}_2$ because there is no ordering on $\mathbb{F}_2$ satisfying @def-ordered-field (you will prove this in @exr-z2-not-ordered below).

However, we can note that $\mathbb{Q}$ is also an ordered field since it satisfies @ax-ordered-field. This means that anything we can prove directly from @ax-field and @ax-ordered-field will apply just as well if our universe were $\mathbb{Q}$ as it would if our universe were $\mathbb{R}.$ In particular, we would have no hope of proving, say, the Intermediate Value Theorem, which is true in $\mathbb{R}$ but not in $\mathbb{Q}.$
